...
### Railway などのクラウド環境でデプロイする場合

Railway、Heroku などのクラウド環境では、`HOST`環境変数を設定しないか、`0.0.0.0`に設定してください：

```bash
# Railway環境変数
PORT=8080  # Railwayが自動的に設定する場合があります
HOST=0.0.0.0  # または設定しない（本番環境では自動的に0.0.0.0が使用されます）
CORS_ORIGIN=https://your-frontend-domain.com
```

**重要**: クラウド環境では特定の IP アドレス（例: `66.33.22.108`）を`HOST`に設定しないでください。`EADDRNOTAVAIL`エラーが発生します。本番環境（`NODE_ENV=production`）では、`HOST`が設定されていない場合、自動的に`0.0.0.0`が使用されます。
